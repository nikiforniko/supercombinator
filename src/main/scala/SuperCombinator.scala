...
object SuperCombinator {
  ...
  def lambdaLifting(term: Term, boundVariables: Set[Var], freeVariables: Set[Var], fromLambda: Boolean): (SCTerm, Set[Var], Set[Var]) = {
    term match {
  ...
  def ApplyNArgs(sp: SCTerm, vars: List[SCVar]): SCTerm = vars.headOption.fold(sp)(x => ApplyNArgs(SPAppl(sp, x), vars.tail))
}
...
object SPReduce {
  ...
  def combMuNu(term: SCTerm ): Option[SCTerm] = {
      ...
      val l = List[SCTerm => Option[SCTerm]](combApply, mu(combMuNu), nu(combMuNu))
      l.foldLeft[Option[SCTerm]]((None))((x, y) =>
        x match {
          case None => y(term)
          case default => default
        }
