...
object GCodeParser {
  ...
  def pushBoolToken[_: P] = P("PUSHBOOL" ~ whitespaceToken ~ ("TRUE" | "FALSE").!).map {
    case "TRUE" => PushBool(true)
    case "FALSE" => PushBool(false)
  }

  def instructionWithStringToken[_: P] = P("PUSHGLOBAL" ~ whitespaceToken ~ letter.rep.!).map(PushGlobal)

  def globalStartToken[_: P] = P("GLOBSTART" ~ whitespaceToken ~ letter.rep.! ~ digit.rep(1).!).map {
  ...
  def gCodeToken[_: P] = P(mainFunc | globalStartFunc)
  def gCode[_: P] = P(gCodeToken.rep(1, sep = ws))

  def parseGCode(code: String) = parse(code, gCode(_)).fold(
    (stack, index, extra) =>
      s"Failure! at index: $index found: ... ${Util.literalize(extra.input.slice(index, index + 15))} expected: ${extra.trace().label}",
    (v, _) => s"Parsed: $v")
